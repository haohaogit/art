...
public class M_productMaintenceController {
	...
	public String productMaintencebottomSave(@PathVariable int id ,HttpServletRequest request){
		...
		int isok = typeMapper.updateByPrimaryKeySelective(type1);
		if(isok==1){
		...
		if(!isExist){
			...
			isok = bottomCaseTypeMapper.insertSelective(bottomCaseType);
			if(isok==1){
	...
	public String productMaintenceCodeSave(@PathVariable int id,HttpServletRequest request){
		...
		if(!isExist){
			...
			isok = outLineTypeMapper.insertSelective(outLineType);
			if(isok==1){
		...
		if(!isExist){
			...
			isok = colorTypeMapper.insertSelective(colorType);
			if(isok==1){
		...
		if(!isExist){
			...
			partType.setPbid(id);
			isok = partTypeMapper.insertSelective(partType);
			if(isok==1){
		...
		if(!isExist){
			...
			isok = textureTypeMapper.insertSelective(textureType);
			if(isok==1){
	...
	public String productMaintenceSetParaSave(@PathVariable int id,HttpServletRequest request){
		...
		survey.setTid(id);
		isok = surveyMapper.updateOnlyCA(survey);
		if(isok==1){
			request.setAttribute("status", "200");
		}else {
			request.setAttribute("status", "100");
	...
	public String produceNewaddyuyi(HttpServletRequest request){
		...
		int isok = 0;
		Type type = new Type();
		type = typeMapper.selectAllByName(tname);
		if(tname.equals(type.getTname())){
			...
			isok = typeMapper.insertSelective(type);
			if(isok==1){
				request.setAttribute("status", 200);
			}else{
				request.setAttribute("status", 100);
			...
			session.setAttribute("tid", typeMapper.selectAllByName(tname).getTid());
