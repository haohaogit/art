...
//		 $("#caseimg").val(fileArray[0]);
//		for(var i=1;i<fileArray.length;i++){
//			 $("#caseimg"+i).val(fileArray[i]);
//		}
...
//		'" style="margin-top: 12px; width: 125px;"/></a>');
	...
	//	if(oldfilename!=""){
//		fileArray.push(oldfilename);
//	}
	for(var i=1;i<5;i++){
		oldfilename=$('#caseimg'+i).val();
		if(oldfilename!=""){
...
//						$('#filename').html('<a class=" " href="'+contextPath+'/file/download?filename='+encodeURIComponent(filename)+'">'+filename+'</a>');
						if(number=="0"||number==0){
							$( '#caseimg' ).val(filename);  
						}else
							{
							$( '#caseimg'+number ).val(filename);  
							}
		 	   });  
